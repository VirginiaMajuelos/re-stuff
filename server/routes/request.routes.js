...
router.get("/all-requests", (req, res) => {
  Request.find()
    ...
    .catch((err) =>
      res.status(500).json({ err, errMessage: "Error looking for request" })
    );
});
...
router.post("/create-request", (req, res) => {
  ...
  Request.create(query)
    ...
    .catch((err) =>
      res.status(500).json({ err, errMessage: "Error creating a request" })
    );
});
...
router.put("/edit-request-status/:id", (req, res) => {
  ...
  Request.findByIdAndUpdate(id, { isAccept, idProduct }, { new: true })
    ...
    .catch((err) =>
      res.status(500).json({ err, errMessage: "Error accepting request" })
    );
});
...
router.delete("/delete-request/:id", (req, res) => {
  ...
  Request.findByIdAndDelete(id)
    ...
    .catch((err) =>
      res.status(500).json({ err, errMessage: "Error to delete request" })
    );
});
